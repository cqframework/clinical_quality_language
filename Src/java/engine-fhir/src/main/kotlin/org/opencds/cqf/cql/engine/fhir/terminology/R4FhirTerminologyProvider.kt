...
/**
 * @param fhirClient - an IGenericClient that has endpoint and authentication already defined and
 *   set.
 */
class R4FhirTerminologyProvider(val fhirClient: IGenericClient) : TerminologyProvider {
    ...
    override fun lookup(code: Code, codeSystem: CodeSystemInfo): Code {
        try {
            ...
            val display = respParam.getParameter("display").getValue() as StringType?
            if (display != null) {
                code.withDisplay(display.value)
            }
