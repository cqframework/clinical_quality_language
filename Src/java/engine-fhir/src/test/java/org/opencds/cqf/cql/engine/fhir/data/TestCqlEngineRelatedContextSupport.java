...
public class TestCqlEngineRelatedContextSupport extends FhirExecutionTestBase {
    ...
    private static final Patient PATIENT_123 =
    ...
    private static final Patient PATIENT_ABC =
            getPatient("abc", LocalDate.of(1970, Month.MARCH, 21), PRACTITIONER_ZULU);
    private static final Patient PATIENT_DEF =
            getPatient("def", LocalDate.of(1975, Month.AUGUST, 21), PRACTITIONER_ZULU);

    private static final RetrieveProvider retrieveProvider =
            (context,
                    contextPath,
                    contextValue,
                    dataType,
                    ...
                    dateHighPath,
                    dateRange) -> {
                final Set<Patient> allPatients =
                        Set.of(PATIENT_123, PATIENT_456, PATIENT_789, PATIENT_ABC, PATIENT_DEF);
                ...
                if (PATIENT.equals(dataType)
                        && PATIENT.equals(context)
                        && ID.equals(contextPath)
                        && _PATIENT_123.equals(contextValue)) {
                ...
                if (PRACTITIONER.equals(dataType)
                    ...
                    if (optPatient123.isPresent()) {
                        final List<String> generalPractitionerIds =
                                optPatient123.get().getGeneralPractitioner().stream()
                ...
                if (PATIENT.equals(dataType)
                        && PRACTITIONER.equals(context)
                        && GENERAL_PRACTITIONER.equals(contextPath)
                        && equals) {
                    logger.info(">>> patients for practitioner xyz");
                    return allPatients.stream()
                            .filter(patient -> getMatchingPractitioners(patient).contains(PRACTITIONER_XYZ.getId()))
                            .collect(Collectors.toList());
                }
    ...
    public void testCqlEngineRelatedContext() {
        ...
        final List<Patient> patientsForPractitioner = ((List<?>) resultAllPatientForGp)
                .stream()
        ...
        assertThat(
                patientsForPractitioner.stream().map(Patient::getId).collect(Collectors.toSet()),
                is(Set.of(PATIENT_123, PATIENT_456, PATIENT_789).stream()
                        .map(Patient::getId)
    ...
    private static Practitioner getPractitioner(String practitionerId, String firstName, String lastName) {
        ...
        practitioner.setName(List.of(new HumanName().setFamily(lastName).setGiven(List.of(new StringType(firstName)))));
    ...
    private static Patient getPatient(
            String patientId, LocalDate birthDateLocalDate, Practitioner nullablePractitioner) {
        final Patient patient = new Patient();

        patient.setId(patientId);

        patient.setBirthDate(Date.from(
                birthDateLocalDate.atStartOfDay(ZoneId.systemDefault()).toInstant()));

        if (nullablePractitioner != null) {
            patient.setGeneralPractitioner(
                    List.of(new Reference().setReference(PRACTITIONER_SLASH + nullablePractitioner.getId())));
        }
