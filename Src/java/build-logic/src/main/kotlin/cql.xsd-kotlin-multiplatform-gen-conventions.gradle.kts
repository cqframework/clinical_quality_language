...
kotlin { sourceSets { commonMain { kotlin { srcDir(destDir) } } } }

val xsdKotlinGenTask = tasks.register<XsdKotlinGenTask>("runXsdKotlinGen")

tasks.withType<XsdKotlinGenTask>().configureEach { mustRunAfter(tasks.withType<Delete>()) }

val suppressGeneratedWarnings =
    tasks.register("suppressGeneratedWarnings") {
        ...
        doLast {
            ...
            if (generatedDir.exists()) {
                ...
                fileTree(generatedDir) { include("**/*.kt") }
                    .files
                    .forEach { f ->
                        var text = f.readText()
...
tasks.named("clean").configure { doLast { delete(destDir) } }
