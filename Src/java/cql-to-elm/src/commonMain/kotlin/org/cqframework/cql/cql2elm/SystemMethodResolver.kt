...
class SystemMethodResolver(
    ...
    private fun checkArgumentCount(
        ...
        require(actualCount == expectedCount) {
            "Expected $expectedCount argument for method $functionName."
        }
    }
    ...
    fun resolveMethod(
        ...
        return when (functionName) {
            ...
            "all" -> {
                ...
                val returnClause = of.createReturnClause()
                returnClause.expression = builder.createLiteral(true)
            ...
            "exists" -> {
                if (ctx?.expression() == null || ctx.expression().isEmpty()) {
                    val params: kotlin.collections.List<Expression> = getParams(target, ctx)
