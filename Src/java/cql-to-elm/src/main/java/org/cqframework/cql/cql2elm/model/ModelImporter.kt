...
class ModelImporter(val modelInfo: ModelInfo, val modelManager: ModelManager?) {
    ...
    private fun resolveTypeName(typeName: String): DataType? {
        ...
        if (typeName.lowercase(Locale.getDefault()).startsWith("interval<")) {
            val pointType =
                resolveTypeName(
    ...
    private fun resolveOpenType(classType: ClassType, e: ClassInfoElement): DataType {
        ...
        elementType =
            TypeParameter(parameterTypeSpecifier.parameterName, TypeParameterConstraint.TYPE, null)
        return elementType
    }
        ...
        if (result == null) {
            result =
                if (t is ProfileInfo) {
                    ProfileType(
                        qualifiedName,
                        resolveTypeNameOrSpecifier(t.getBaseType(), t.getBaseTypeSpecifier())
                    )
                } else {
            ...
            if (t.parameter != null) {
                result.addGenericParameter(resolveGenericParameterDeclarations(t.parameter))
            }
