...
class LibraryInfo : BaseInfo() {
    ...
    override var definition: cqlParser.LibraryDefinitionContext? = null

    fun withDefinition(value: cqlParser.LibraryDefinitionContext?): LibraryInfo {
    ...
    fun resolveContext(ctx: cqlParser.ContextDefinitionContext): ContextDefinitionInfo? {
        for (cd in contextDefinitions) {
            if (ctx.sourceInterval.equals(cd.definition?.sourceInterval)) {
                return cd
            }
        }
        return null
