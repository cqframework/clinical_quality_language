 ...
 * Specifically: model conversions (FHIRHelpers.ToDateTime etc.) must be recorded for choice-typed
 * operands in operators like In/Contains.
 */
class FhirChoiceResolutionTest {
    ...
    fun choicePropertyInMembershipResolvesWithModelConversion() {
        ...
        val testDef =
            library.statements.filterIsInstance<org.hl7.cql.ast.ExpressionDefinition>().first {
                it.name.value == "Test"
            }
        ...
        assertTrue(
            ...
            "Resolution should have conversions for choice type",
        )
        ...
        println(
            "Left conversion: " +
                "isCast=${leftConversion.isCast}, " +
                "operator=${leftConversion.operator?.name}, " +
                "inner=${leftConversion.conversion?.operator?.name}"
        )
    }
}
