...
class BaseTest {
    ...
    fun choiceWithAlternativeConversion() {
        val def = TestUtils.visitFile("fhir/stu3/TestChoiceTypes.cql") as ExpressionDef?
    ...
    fun uriConversion() {
        ...
        TestUtils.visitFile("fhir/stu3/TestURIConversion.cql") as ExpressionDef?
    }

    @Test
    @Throws(IOException::class)
    fun fhirTiming() {
        val def = TestUtils.visitFile("fhir/stu3/TestFHIRTiming.cql") as ExpressionDef?
        // Query->
        //  where->
        //      In->
        //          left->
        ...
        val request = source.expression as Retrieve?
        MatcherAssert.assertThat(request!!.dataType, QuickDataType.quickDataType("Procedure"))
    ...
    fun equalityWithConversions() {
        val library = TestUtils.visitFileLibrary("fhir/stu3/EqualityWithConversions.cql")
    ...
    fun doubleListPromotion() {
        val translator = TestUtils.runSemanticTest("fhir/stu3/TestDoubleListPromotion.cql", 0)
    ...
    fun conceptConversion() {
        val translator = TestUtils.runSemanticTest("fhir/stu3/TestConceptConversion.cql", 0)
        val library = translator.toELM()
        val defs: MutableMap<String?, ExpressionDef> = HashMap()
    ...
    fun retrieveWithConcept() {
        val translator = TestUtils.runSemanticTest("fhir/stu3/TestRetrieveWithConcept.cql", 0)
