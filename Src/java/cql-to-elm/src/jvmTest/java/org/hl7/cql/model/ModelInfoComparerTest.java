...
public class ModelInfoComparerTest {
    ...
    void compareModelInfo() {
        ...
        assertThat(
                ...
                is(String.format("ModelInfo.Type info allowedUnits in left only%n"
    ...
    void compareNewModelInfo() {
        ...
        assertThat(
                differences.toString(),
                is(String.format("ModelInfo.DeviceRequest.primaryCodePath: codeCodeableConcept <> code%n"
    ...
    void compareMetadataModelInfo() {
        ...
        assertThat(
                differences.toString(),
                is(String.format("ModelInfo.markdown.Element value in right only%n" + // redeclaration for metadata
                        "ModelInfo.SimpleQuantity.primaryCodePath: code <> null%n"
                        + // primaryCodePath should not be set on a non-retrievable type
                        "ModelInfo.SimpleQuantity.Element value in left only%n"
                        + // SimpleQuantity is derived from Quantity, no need to re-declare elements
                        "ModelInfo.SimpleQuantity.Element unit in left only%n"
                        + // ditto
                        "ModelInfo.SimpleQuantity.Element system in left only%n"
                        + // ditto
                        "ModelInfo.SimpleQuantity.Element code in left only%n"
                        + // ditto
                        "ModelInfo.MoneyQuantity.primaryCodePath: code <> null%n"
                        + // primaryCodePath should not be set on a non-retrievable type
                        "ModelInfo.MoneyQuantity.Element value in left only%n"
                        + // MoneyQuantity is derived from Quantity, no need to re-declare elements
                        "ModelInfo.MoneyQuantity.Element comparator in left only%n"
                        + // ditto
                        "ModelInfo.MoneyQuantity.Element unit in left only%n"
                        + // ditto
                        "ModelInfo.MoneyQuantity.Element system in left only%n"
                        + // ditto
                        ...
                        + // ditto
                        "ModelInfo.ElementDefinition.Type.versioning.type: FHIR.ReferenceVersionRules <> List<FHIR.canonical>%n"
                        + // ditto
                        "ModelInfo.ElementDefinition.Type.Element aggregation in right only%n"
                        + // ditto
    ...
    public static void compareModelInfo(ModelInfoCompareContext context, ModelInfo a, ModelInfo b) {
        ...
        compareAttribute(
                context, "patientClassIdentifier", a.getPatientClassIdentifier(), b.getPatientClassIdentifier());
        compareAttribute(
                context,
                "patientBirthDatePropertyName",
                a.getPatientBirthDatePropertyName(),
                b.getPatientBirthDatePropertyName());
        ...
        // typeInfo
        Map<String, TypeInfo> tia = a.getTypeInfo().stream()
                .filter(x -> x instanceof ClassInfo || x instanceof SimpleTypeInfo)
                .collect(Collectors.toMap(
                        k -> k instanceof ClassInfo ? ((ClassInfo) k).getName() : ((SimpleTypeInfo) k).getName(),
                        v -> v));
        Map<String, TypeInfo> tib = b.getTypeInfo().stream()
                .filter(x -> x instanceof ClassInfo || x instanceof SimpleTypeInfo)
                .collect(Collectors.toMap(
                        k -> k instanceof ClassInfo ? ((ClassInfo) k).getName() : ((SimpleTypeInfo) k).getName(),
                        v -> v));
    ...
    public static String descriptor(IntervalTypeSpecifier intervalTypeSpecifier) {
        if (intervalTypeSpecifier != null) {
            return String.format(
                    "Interval<%s>",
                    descriptor(intervalTypeSpecifier.getPointType(), intervalTypeSpecifier.getPointTypeSpecifier()));
        }
    ...
    public static String descriptor(ListTypeSpecifier listTypeSpecifier) {
        if (listTypeSpecifier != null) {
            return String.format(
                    "List<%s>",
                    descriptor(listTypeSpecifier.getElementType(), listTypeSpecifier.getElementTypeSpecifier()));
        }
    ...
    public static String descriptor(ListTypeInfo listTypeInfo) {
        if (listTypeInfo != null) {
            return String.format(
                    "List<%s>", descriptor(listTypeInfo.getElementType(), listTypeInfo.getElementTypeSpecifier()));
        }
    ...
    public static String descriptor(IntervalTypeInfo intervalTypeInfo) {
        if (intervalTypeInfo != null) {
            return String.format(
                    "Interval<%s>",
                    descriptor(intervalTypeInfo.getPointType(), intervalTypeInfo.getPointTypeSpecifier()));
        }
    ...
    public static void compareTypeInfo(ModelInfoCompareContext context, ClassInfo a, ClassInfo b) {
        ...
        try {
            compareAttribute(
                    context,
                    "baseType",
                    descriptor(a.getBaseType(), a.getBaseTypeSpecifier()),
                    descriptor(b.getBaseType(), b.getBaseTypeSpecifier()));
            ...
            for (int i = 0; i < Math.max(a.getElement().size(), b.getElement().size()); i++) {
                if (i >= a.getElement().size()) {
                    context.append(String.format(
                            "Element %s in right only", b.getElement().get(i).getName()));
                } else if (i >= b.getElement().size()) {
                    context.append(String.format(
                            "Element %s in left only", a.getElement().get(i).getName()));
                } else {
                    compareClassInfoElement(
                            context, a.getElement().get(i), b.getElement().get(i));
                }
            }
        } finally {
            context.popFocus();
    ...
    public static void compareClassInfoElement(
            ModelInfoCompareContext context, ClassInfoElement a, ClassInfoElement b) {
        context.pushFocus(a.getName());
        try {
            ...
            compareAttribute(
                    context,
                    "type",
                    descriptor(a.getElementType(), a.getElementTypeSpecifier()),
                    descriptor(b.getElementType(), b.getElementTypeSpecifier()));
            compareAttribute(context, "target", a.getTarget(), b.getTarget());
    ...
    public static void compareTypeInfo(ModelInfoCompareContext context, TypeInfo a, TypeInfo b) {
        if (a == null) {
            ...
            context.append(String.format(
                    ...
                    descriptor(a), a.getClass().getSimpleName(), descriptor(b)));
