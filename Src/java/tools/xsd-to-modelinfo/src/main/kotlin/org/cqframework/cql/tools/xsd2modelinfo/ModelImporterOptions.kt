...
class ModelImporterOptions {
     ...
     *
     *  * **USE_BASETYPE**: Replace all instances of the simple type with its base type (e.g., a restriction on
     * xsd:string simply becomes System.String in the modelinfo).  This is the default.
     *  * **EXTEND_BASETYPE**: Create a new simple type representation that simply extends the base type.  This
     * maintains the distinction of the restriction type, but still doesn't retain the actual restriction
     * constraints.  Using this option may require CQL authors to perform additional casts.
     *  * **IGNORE**: Create a new simple type with no relation to the base type.  This is rarely the intended
     * behavior.
     *
     */
     ...
     *
     *  * **RENAME_INVALID_REDECLARATIONS**: Rename the element so it is a new element and does not clash with the
     * basetype element.  The element will be renamed by appending the redeclared type to the name using CamelCase
     * (e.g., if element "foo" is redeclared to type "bar", and this is invalid, it will be renamed "fooBar").
     *  * **DISCARD_INVALID_REDECLARATIONS**: Discard the redeclaration, falling back to the originally declared
     * element type and cardinality.
     ...
     *  * **INCLUDE_DEPRECATED**: Include deprecated elements in addition to elements for the latest version.
     *
     */
     ...
     */
    @JvmField
    var normalizePrefix: String? = null
     ...
     */
    @JvmField
    val typeMap: MutableMap<QName?, ModelImporterMapperValue> = HashMap<QName?, ModelImporterMapperValue>()
    ...
    fun getSimpleTypeRestrictionPolicy(): SimpleTypeRestrictionPolicy {
        return (if (simpleTypeRestrictionPolicy != null)
    ...
    fun getElementRedeclarationPolicy(): ElementRedeclarationPolicy {
        return (if (elementRedeclarationPolicy != null)
    ...
    fun withElementRedeclarationPolicy(elementRedeclarationPolicy: ElementRedeclarationPolicy?): ModelImporterOptions {
        this.elementRedeclarationPolicy = elementRedeclarationPolicy
        return this
    }
    ...
    fun applyProperties(properties: Properties) {
        ...
        for (p in properties.stringPropertyNames().stream().sorted().collect(Collectors.toList())) {
            ...
            if (matcher.matches()) {
                typeMap[QName.valueOf(matcher.group(1))] = ModelImporterMapperValue.newRetype(properties.getProperty(p))
                continue
            }

            matcher = EXTEND_PATTERN.matcher(p)
            if (matcher.matches()) {
                typeMap[QName.valueOf(matcher.group(1))] = ModelImporterMapperValue.newExtend(properties.getProperty(p))
                continue
            }
            ...
            if (matcher.matches()) {
                ...
                requireNotNull(value) {
                    String.format(
                        "Class element mapping declared before class mapping: %s",
                        p
                    )
                }
                require(value.relationship != ModelImporterMapperValue.Relationship.RETYPE) {
                    String.format(
                        "Cannot map class elements for retyped classes: %s",
                        p
                    )
                }
        ...
        if (!typeMap.isEmpty()) {
            for (entry in typeMap.entries) {
                ...
                if (value.relationship == ModelImporterMapperValue.Relationship.RETYPE) {
                    properties.setProperty(String.format("retype.%s", key.toString()), value.targetSystemClass)
                } else if (value.relationship == ModelImporterMapperValue.Relationship.EXTEND) {
                    properties.setProperty(String.format("extend.%s", key.toString()), value.targetSystemClass)
    ...
    companion object {
        ...
        private val EXTEND_EL_PATTERN: Pattern = Pattern.compile("\\s*extend\\.([^\\[]+)\\[([^\\]]+)\\]\\s*")
