...
public class GenericClassSignatureParserTest {
    ...
    public void parseTest2() {
        ...
        if(genericClassSignatureParser.isValidGenericSignature()) {
            ...
            assertThat(signature.getGenericParameters().size(), is(2));
            assertThat(signature.getGenericParameters().get(0).getIdentifier(), is("M"));
            assertThat(signature.getGenericParameters().get(0).getConstraint(), is(TypeParameter.TypeParameterConstraint.TYPE));
            assertThat(((ClassType)signature.getGenericParameters().get(0).getConstraintType()).getName(), is("Collection"));
            assertThat(signature.getGenericParameters().get(1).getIdentifier(), is("N"));
        } else {
            fail("Invalid generic class");
        }
    ...
    public void parseTest3() {
        ...
        GenericClassSignatureParser genericClassSignatureParser = new GenericClassSignatureParser("MyType<M extends Collection,N extends Object>", resolvedTypes);
        if(genericClassSignatureParser.isValidGenericSignature()) {
            ClassType signature = genericClassSignatureParser.parseGenericSignature();
            assertThat(signature.getName(), is("MyType"));
            assertThat(signature.getGenericParameters().size(), is(2));
            ...
            assertThat(((ClassType)signature.getGenericParameters().get(0).getConstraintType()).getName(), is("Collection"));
            assertThat(signature.getGenericParameters().get(1).getIdentifier(), is("N"));
            assertThat(signature.getGenericParameters().get(1).getConstraint(), is(TypeParameter.TypeParameterConstraint.TYPE));
            assertThat(((ClassType)signature.getGenericParameters().get(1).getConstraintType()).getName(), is("Object"));
        } else {
            fail("Invalid generic class");
        }
    ...
    public void parseTest4() {
        try {
            ...
            Map<String, DataType> resolvedTypes = new HashMap<>();
            resolvedTypes.put("Collection", collectionType);
            GenericClassSignatureParser genericClassSignatureParser = new GenericClassSignatureParser("MyType<M constrains Collection>", resolvedTypes);
    ...
    public void parseTest5() {
        ...
        GenericClassSignatureParser genericClassSignatureParser = new GenericClassSignatureParser("MyType<M extends List<Object>>", resolvedTypes);
        if(genericClassSignatureParser.isValidGenericSignature()) {
            ClassType signature = genericClassSignatureParser.parseGenericSignature();
            assertThat(signature.getName(), is("MyType"));
            ...
            assertThat(signature.getGenericParameters().get(0).getIdentifier(), is("M"));
            assertThat(signature.getGenericParameters().get(0).getConstraint(), is(TypeParameter.TypeParameterConstraint.TYPE));
            ClassType listOfObject = (ClassType)signature.getGenericParameters().get(0).getConstraintType();
    ...
    public void parseTest6() {
        ...
        GenericClassSignatureParser genericClassSignatureParser = new GenericClassSignatureParser("MyType<M extends Map<String,List<Object>>>", resolvedTypes);
        if(genericClassSignatureParser.isValidGenericSignature()) {
            ClassType signature = genericClassSignatureParser.parseGenericSignature();
            assertThat(signature.getName(), is("MyType"));
            assertThat(signature.getGenericParameters().size(), is(1));
    ...
    public void parseTest7() {
        ...
        resolvedTypes.put("List", listType);
        resolvedTypes.put("Map", mapType);
        GenericClassSignatureParser genericClassSignatureParser = new GenericClassSignatureParser("MyType<M extends Map<String,List<Map<String,Integer>>>>", resolvedTypes);
        if(genericClassSignatureParser.isValidGenericSignature()) {
            ClassType signature = genericClassSignatureParser.parseGenericSignature();
            assertThat(signature.getName(), is("MyType"));
            assertThat(signature.getGenericParameters().size(), is(1));
