    ...
    private static Boolean intervalIn(Object left, Interval right, String precision, State state) {
        ...
        if (left instanceof BaseTemporal) {
            if (AnyTrueEvaluator.anyTrue(Arrays.asList(
                    SameAsEvaluator.sameAs(left, right.getStart(), precision, state),
                    SameAsEvaluator.sameAs(left, right.getEnd(), precision, state)))) {
                return true;
            } else if (AnyTrueEvaluator.anyTrue(Arrays.asList(
                    BeforeEvaluator.before(left, right.getStart(), precision, state),
                    AfterEvaluator.after(left, right.getEnd(), precision, state)))) {
                return false;
            }
            ...
            return AndEvaluator.and(pointSameOrAfterStart, pointSameOrBeforeEnd);
                ...
                EqualEvaluator.equal(left, right.getStart(), state),
                EqualEvaluator.equal(left, right.getEnd(), state)))) {
            return true;
        } else if (AnyTrueEvaluator.anyTrue(Arrays.asList(
                LessEvaluator.less(left, right.getStart(), state),
                GreaterEvaluator.greater(left, right.getEnd(), state)))) {
            return false;
        }
