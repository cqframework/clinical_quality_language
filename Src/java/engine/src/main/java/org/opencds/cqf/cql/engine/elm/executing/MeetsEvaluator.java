...
public class MeetsEvaluator {
    ...
    public static Boolean meetsOperation(Object left, Object right, String precision, State state) {
        ...
        Object maxValue = MaxValueEvaluator.maxValue(
                left != null ? left.getClass().getName() : right.getClass().getName());
        if (left instanceof BaseTemporal && right instanceof BaseTemporal) {
            ...
            // the following blocks adds 1 with the left and check if it is same as right when both params are of type
            // DateTime/Time
            if (left instanceof DateTime && right instanceof DateTime) {
                DateTime dt = new DateTime(
                        ((DateTime) left)
                                .getDateTime()
                                .plus(1, Precision.fromString(precision).toChronoUnit()),
                        ((BaseTemporal) left).getPrecision());
