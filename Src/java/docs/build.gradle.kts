    ...
    tasks.registering {
        ...
        doLast {
                            ...
                            when {
                                ...
                                endMatch != null && currentSnippet == endMatch.groupValues[1] -> {
                                    ...
                                    snippets[currentSnippet!!] =
                                        trimmed
                                            .map {
                                                if (it.length >= minIndent) it.drop(minIndent)
                                                else it
                                            }
                                            .joinToString("\n")
                                    currentSnippet = null
            ...
            if (snippets.isNotEmpty()) {
                println(
                    "Extracted ${snippets.size} snippet(s): ${snippets.keys.joinToString(", ")}"
                )
            }
            ...
            val moduleLinks =
                mutableListOf<Triple<String, String, String>>() // module, docName, path

            val excludedDirs =
                setOf("build", "buildSrc", "docs", "gradle", "config", "kotlin-js-store")
            modulesDir
                .listFiles()
                ...
                ?.forEach { moduleDir ->
                    ...
                    val mdFiles =
                        moduleDir.listFiles()?.filter { it.isFile && it.extension == "md" }
                            ?: emptyList()
                    if (mdFiles.isNotEmpty()) {
                        ...
                        val indexFile =
                            mdFiles.find { it.name.equals("README.md", ignoreCase = true) }
                                ?: mdFiles.first()
                        moduleLinks.add(
                            Triple(
                                moduleDir.name,
                                indexFile.nameWithoutExtension,
                                "modules/${moduleDir.name}/${indexFile.name}",
                            )
                        )
                    }
                }
            ...
            val crossCuttingDocs =
                siteDir
                    .listFiles()
                    ?.filter { it.isFile && it.extension == "md" && it.name != "index.md" }
                    ?.sortedBy { it.name }
                    ?.map { it.nameWithoutExtension to it.name } ?: emptyList()
