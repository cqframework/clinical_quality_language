...
function getAllParentClasses(config) {
  return Object.entries(config.classes)
    .filter(([k, v]) => {
      return Object.values(config.classes).some((_) => k === _.extendsClass);
    })
    .map(([k, v]) => {
      return {
        className: k,
...
function getAllChildClasses(parentClass, config) {
  return Object.entries(config.classes)
    .filter(([k, v]) => v.extendsClass === parentClass)
    .flatMap(([k, v]) => {
      return [
        {
          className: k,
  ...
  return (
    {
      ...
      "xs:integer": "Int",
      "xs:decimal":
        "@kotlinx.serialization.Contextual org.cqframework.cql.elm.serializing.BigDecimal",
      "xs:dateTime": "String",
      "xs:time": "String",
      "xs:date": "String",
      ...
      "xs:anyURI": "String",
      "xs:QName":
        "@kotlinx.serialization.Serializable(org.cqframework.cql.elm.serializing.QNameJsonSerializer::class) nl.adaptivity.xmlutil.QName",
      "xs:token": "String",
      "xs:NCName": "String",
      "xs:ID": "String",
    }[rawType] || rawType
...
function makeLocalName(name) {
  ...
  if (["ModelInfo", "Library"].includes(name)) {
    ...
    for (const element of elements) {
      switch (element.name) {
        ...
        case "xs:complexType":
          ...
          const extendsClass = (extension && extension.attributes.base) || null;
          ...
          if (mode === "WRITE_FILES") {
            const renderGetSet = (field, className, type) => {
              ...
              const name =
                field.attributes.name === "content" &&
                config.namespaceUri === "urn:hl7-org:cql-annotations:r1"
                  ? "s"
...
`;
  ...
  override fun hashCode(): Int {
        // TODO: Implement a proper hashCode function
        return 1
    }
